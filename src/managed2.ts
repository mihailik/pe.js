...
module pe.managed2 {
  ...
  */
  ...
  export class PropertyInfo {
    ...
    getAccessor: MethodInfo = null;
    setAccessor: MethodInfo = null;
  }
  ...
  export class EventInfo {
    name: string;
    addHandler: MethodInfo = null;
    removeHandler: MethodInfo = null;
  }
  ...
  */
  class AssemblyReading {
    ...
    private _createAssemblyFromTables() {
      ...
      var assemblyTable = this.managedHeaders.tableStream.tables[0x20]; // Assembly
      if (!assemblyTable || !assemblyTable.length)
        return;

      var assemblyRow: tables.Assembly = assemblyTable[0];

      var moduleTable = this.managedHeaders.tableStream.tables[0x00]; // Module
      var typeDefTable = this.managedHeaders.tableStream.tables[0x02]; // TypeDef
      var fieldTable = this.managedHeaders.tableStream.tables[0x04]; // Field
      var methodDefTable = this.managedHeaders.tableStream.tables[0x06]; // MethodDef
      ...
      var assemblyRefTable: tables.AssemblyRef[] = this.managedHeaders.tableStream.tables[0x23];  // AssemblyRef
      if (assemblyRefTable) {
      ...
      for (var i = 0; i < typeDefTable.length; i++) {
        ...
        this.managedHeaders.tableStream.allFields.length;
    ...
    private _getMscorlibIfThisShouldBeOne(): Assembly {
      ...
      var typeDefTable: tables.TypeDef[] = this.managedHeaders.tableStream.tables[0x02]; // 0x02
      if (!typeDefTable)
        return null;
    ...
    readMetadataStreams() {
      this.managedHeaders.metadataStreams.read(
        this.managedHeaders.clrDirectory.metadataDir.address,
        this.managedHeaders.clrMetadata.streamCount,
        this.reader);
    }
    ...
    readTableStream() {
      ...
      this.managedHeaders.tableStream.read(
        this.reader,
        this.managedHeaders.metadataStreams.strings.size,
        this.managedHeaders.metadataStreams.guids.length,
        this.managedHeaders.metadataStreams.blobs.size);
    }
